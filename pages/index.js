import Head from 'next/head';
import styles from '../styles/Home.module.css';

export default function Home() {
  return (
    <>
      <header id="header">
        <div className="container">
          <div className="row">
            <div className="header-left">
              <a href="home.html" className="header-link">
                <img src="./assets/images/logos/logo.png" />
              </a>
              <nav>
                <ul className="nav-list">
                  <li>
                    <a href="home.html"> Ana Səhife </a>
                  </li>
                  <li>
                    <a href="blog.html"> Bloq </a>
                  </li>
                  <li>
                    <a href="qanda.html"> Sual&amp;Cavab </a>
                  </li>
                  <li>
                    <a href="about.html">Haqqımızda </a>
                  </li>
                  <li>
                    <a href="contact.html"> Əlaqə</a>
                  </li>
                  <li>
                    <a href="rules.html"> İstifadə Şərtləri </a>
                  </li>
                </ul>
              </nav>
            </div>
            <div className="header-right">
              <a href="./install.html" className="download">
                {' '}
                Yüklə{' '}
              </a>
              <div className="open-btn">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width={33}
                  height={33}
                  viewBox="0 0 24 24"
                  style={{ fill: 'rgba(255, 255, 255)' }}
                >
                  <path d="M4 6h16v2H4zm0 5h16v2H4zm0 5h16v2H4z" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </header>
      <main>
        <div className="mobil-menu">
          <div className="mobil-container">
            <nav>
              <ul className="listMobil">
                <li>
                  <a className="nav-link" href="home.html">
                    {' '}
                    Ana Səhifə{' '}
                  </a>
                </li>
                <li>
                  <a className="nav-link" href="blog.html">
                    {' '}
                    Bloq{' '}
                  </a>
                </li>
                <li>
                  <a className="nav-link" href="qanda.html">
                    {' '}
                    Sual&amp;Cavab{' '}
                  </a>
                </li>
                <li>
                  <a className="nav-link" href="about.html">
                    Haqqımızda{' '}
                  </a>
                </li>
                <li>
                  <a className="nav-link" href="contact.html">
                    {' '}
                    Əlaqə
                  </a>
                </li>
                <li>
                  <a className="nav-link" href="rules.html">
                    {' '}
                    İstifadə Şərtləri{' '}
                  </a>
                </li>
              </ul>
            </nav>
          </div>
        </div>
        <div className="bg-elements">
          <div className="bottom">
            {/*?xml version="1.0" encoding="UTF-8"?*/}
            <svg
              width="100%"
              height="60px"
              viewBox="0 0 1440 56"
              preserveAspectRatio="none"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <g
                id="Waves/wave-bottom"
                stroke="none"
                strokeWidth={1}
                fill="none"
                fillRule="evenodd"
              >
                <path
                  d="M1440,18.6164191 L1440,56 L0,56 L0,19.4091149 C115.88527,18.914036 229.081162,52 655.578314,52 C1082.07547,52 1250.37725,17.9354374 1440,18.6164191 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.5"
                />
                <path
                  d="M1440,0.616419131 L1440,56 L0,56 L0,6.40911495 C106.226448,5.95529994 313.375622,56 623.178314,56 C932.981007,56 1268.35502,-9.23705556e-14 1440,0.616419131 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.25"
                />
                <path
                  d="M1440,31.6164191 L1440,56 L0,56 L0,35.4091149 C95.7633114,35 278.134957,56 578.778314,56 C879.421672,56 1135.89541,30.5243052 1440,31.6164191 L1440,31.6164191 Z"
                  id="base"
                  fill="#FFFFFF"
                />
              </g>
            </svg>
          </div>
        </div>
        <section id="banner" className="banner">
          <div className="container">
            <div className="row">
              <div className="banner-left">
                <a href="com.reminant.mobile://salam">
                  <span>GEYİM TƏRZİ AZADLIĞI ÜÇÜN PROQRAM</span>
                </a>
                <h1>İndi öz dizaynını yarat !</h1>
                <p>
                  Reminant sizə bir neçə dəqiqə ərzində öz paltarlarınızı dizayn
                  etmək imkanı yaradır.Proqramı yüklə və öz fərqliliyini yarat
                </p>
                <div className="btn-flex">
                  <a className="android" href="./install.html">
                    {/*?xml version="1.0" encoding="UTF-8"?*/}
                    <svg
                      width="170px"
                      height="50px"
                      viewBox="0 0 170 50"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlnsXlink="http://www.w3.org/1999/xlink"
                    >
                      {/* Generator: Sketch 55.2 (78181) - https://sketchapp.com */}
                      <title>Store Badges/google-play-white</title>
                      <desc>Created with Sketch.</desc>
                      <defs>
                        <path
                          d="M5.95503704,50 C2.67214815,50 0,47.34 0,44.08375 L0,5.9075 C0,2.65 2.67214815,0 5.95503704,0 L163.951778,0 C167.233407,0 170,2.65 170,5.9075 L170,44.08375 C170,47.34 167.233407,50 163.951778,50 L5.95503704,50 Z"
                          id="path-1"
                        />
                      </defs>
                      <g
                        id="Store-Badges/google-play-white"
                        stroke="none"
                        strokeWidth={1}
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g id="Google-Play-Badge">
                          <mask id="mask-2" fill="white">
                            <use xlinkHref="#path-1" />
                          </mask>
                          <use id="Mask" fill="#FFFFFF" xlinkHref="#path-1" />
                          <path
                            d="M151.49107,41.7716915 L149.059621,41.7716915 L151.433595,36.5064684 L147.271644,27.0393125 L149.783058,27.0393125 L152.556859,33.8113839 L152.631826,33.8113839 L155.304421,27.0393125 L157.897049,27.0393125 L151.49107,41.7716915 L151.49107,41.7716915 Z M144.860187,35.8030226 L144.780222,35.8030226 C144.140945,36.8162582 143.001228,37.4016736 141.805258,37.3311118 C140.832387,37.4033509 139.8717,37.0787381 139.142032,36.4312174 C138.412365,35.7836967 137.975854,34.8684087 137.931933,33.8938482 C137.931933,31.6035887 140.203451,30.3778687 142.429988,30.3778687 C143.277645,30.3757657 144.112755,30.5826121 144.861437,30.9801083 L144.861437,30.8201774 C144.735847,29.5901136 143.640843,28.6921208 142.409997,28.8097968 C141.535301,28.7344576 140.69901,29.1842396 140.279668,29.9555514 L138.210563,29.0909253 C138.997314,27.5369338 140.631245,26.598 142.370014,26.7007089 C145.042608,26.7007089 147.117961,28.24754 147.117961,31.020091 L147.117961,37.0087512 L144.868933,37.0087512 L144.868933,35.8030226 L144.860187,35.8030226 L144.860187,35.8030226 Z M142.729858,32.2670517 C141.584104,32.2670517 140.258427,32.7293518 140.258427,33.9350804 C140.410733,34.7980563 141.220125,35.3846444 142.087636,35.2607571 C143.499384,35.2450794 144.683015,34.1899998 144.860187,32.7893259 C144.209732,32.4274207 143.473946,32.2470344 142.729858,32.2670517 L142.729858,32.2670517 L142.729858,32.2670517 Z M134.30975,21.4154945 L136.638743,21.4154945 L136.638743,37.0349898 L134.307251,37.0349898 L134.307251,21.416744 L134.30975,21.4154945 L134.30975,21.4154945 Z M127.900022,31.1175489 L124.64393,31.1175489 L124.64393,37.0349898 L122.313687,37.0349898 L122.313687,21.416744 L127.900022,21.416744 C129.697254,21.316782 131.402439,22.2187799 132.33116,23.7606992 C133.25988,25.3026185 133.25988,27.2316743 132.33116,28.7735936 C131.402439,30.3155129 129.697254,31.2175108 127.900022,31.1175489 L127.900022,31.1175489 L127.900022,31.1175489 Z M127.959996,23.5895543 L124.64393,23.5895543 L124.64393,28.943489 L127.959996,28.943489 C128.930171,28.9644502 129.835756,28.4588571 130.326995,27.6219804 C130.818235,26.7851036 130.818235,25.7479398 130.326995,24.911063 C129.835756,24.0741862 128.930171,23.5685932 127.959996,23.5895543 L127.959996,23.5895543 L127.959996,23.5895543 Z M112.675356,35.263256 C113.745483,35.2687796 114.739492,34.7105394 115.291724,33.7938915 L117.103441,35.002119 C116.117632,36.48215 114.453617,37.3672143 112.675356,37.3573504 C111.263495,37.3777077 109.904053,36.8230642 108.90939,35.8208607 C107.914727,34.8186572 107.370367,33.4550655 107.401386,32.0433985 C107.328974,30.6655143 107.823612,29.3180221 108.770291,28.3142259 C109.71697,27.3104297 111.033209,26.7377885 112.412969,26.7294465 C114.489527,26.787608 116.307745,28.1389117 116.962252,30.1104844 L117.203397,30.7139734 L110.09772,33.653952 C110.545987,34.6678995 111.567551,35.3056956 112.675356,35.263256 L112.675356,35.263256 L112.675356,35.263256 Z M114.465831,29.9093213 C114.093761,29.1722381 113.316738,28.7293498 112.492934,28.7848076 C111.710817,28.8200937 110.975771,29.1685109 110.453317,29.7516007 C109.930863,30.3346906 109.664924,31.1034324 109.715386,31.8847171 L114.465831,29.9093213 L114.465831,29.9093213 Z M103.937884,21.4129956 L106.276873,21.4129956 L106.276873,37.0349898 L103.941632,37.0349898 L103.941632,21.416744 L103.937884,21.4129956 L103.937884,21.4129956 Z M97.280763,42.1277875 C95.2175856,42.1384522 93.3542191,40.8963727 92.5703,38.9878953 L94.6031709,38.1420111 C95.0289406,39.2576525 96.0869789,40.005142 97.280763,40.0336931 C99.0300065,40.0336931 100.119535,38.9466631 100.119535,36.9100439 L100.119535,36.1453746 L100.029574,36.1453746 C99.3199718,36.9397219 98.2968057,37.3816163 97.2320341,37.3536021 C94.3867898,37.2301726 92.1436087,34.8875703 92.1436087,32.0396501 C92.1436087,29.1917298 94.3867898,26.8491276 97.2320341,26.7256981 C98.2919159,26.700318 99.3113398,27.132856 100.029574,27.9126848 L100.110789,27.9126848 L100.110789,27.0468092 L102.324832,27.0468092 L102.324832,36.5889327 C102.32858,40.5172339 100.013331,42.1277875 97.280763,42.1277875 L97.280763,42.1277875 L97.280763,42.1277875 Z M97.4419433,28.8222914 C95.7702029,28.9659548 94.4863479,30.3648718 94.4863479,32.0427737 C94.4863479,33.7206757 95.7702029,35.1195926 97.4419433,35.263256 C99.0569943,35.0481199 100.262855,33.6699002 100.261591,32.0405839 C100.260326,30.4112676 99.0523284,29.0349209 97.4369455,28.8222914 L97.4419433,28.8222914 L97.4419433,28.8222914 Z M99.835908,15.9191214 L96.1874858,10.0679018 L96.2187223,11.1224458 L96.2187223,15.9191214 L95.0054969,15.9191214 L95.0054969,8.17247147 L96.3724058,8.17247147 L99.852151,13.7413132 L99.822164,12.6855198 L99.822164,8.17247147 L101.029142,8.17247147 L101.029142,15.9191214 L99.8296607,15.9191214 L99.835908,15.9191214 L99.835908,15.9191214 Z M89.9251939,16.086549 C88.8563149,16.1112835 87.8263874,15.6849815 87.087671,14.9120569 C85.5682608,13.3032575 85.5682608,10.7883354 87.087671,9.17953597 C87.8381682,8.42453548 88.8587674,8 89.9233197,8 C90.9878721,8 92.0084713,8.42453548 92.7589684,9.17953597 C94.2815027,10.7851615 94.2815027,13.3014335 92.7589684,14.9070591 C92.0204707,15.6816261 90.9901475,16.1097181 89.9201961,16.086549 L89.9251939,16.086549 L89.9251939,16.086549 Z M87.9785358,9.98168907 C86.9193832,11.1520421 86.9193832,12.9345529 87.9785358,14.104906 C89.055542,15.1777887 90.7973447,15.1777887 91.874351,14.104906 C92.9323135,12.9331719 92.9296121,11.1502118 91.8681037,9.98168907 C90.7965232,8.90895059 89.055114,8.90895059 87.9785358,9.98168907 L87.9785358,9.98168907 Z M91.1696557,32.0433985 C91.1777483,34.202578 89.8833182,36.1536544 87.8908144,36.98555 C85.8983107,37.8174457 83.6008153,37.3660439 82.0711703,35.8421319 C80.5415253,34.31822 80.0814996,32.0224358 80.9059077,30.0268224 C81.7303157,28.0312089 83.6765178,26.7294617 85.8357124,26.7294465 C87.2530734,26.7073934 88.6190698,27.2596784 89.6228278,28.2606122 C90.6265858,29.261546 91.1827174,30.6259809 91.1646579,32.0433985 L91.1696557,32.0433985 L91.1696557,32.0433985 Z M85.8357124,28.8222914 C84.5123283,28.7278169 83.2658674,29.4525619 82.6933151,30.6494139 C82.1207628,31.846266 82.33869,33.2715477 83.2427217,34.2426284 C84.1467534,35.2137091 85.5528299,35.5328781 86.7875189,35.0472712 C88.0222079,34.5616643 88.8341284,33.3701505 88.8344156,32.0433985 C88.8775728,31.2141675 88.5821524,30.4029572 88.0158558,29.7956724 C87.4495592,29.1883876 86.660936,28.8370919 85.8307146,28.8222914 L85.8357124,28.8222914 L85.8357124,28.8222914 Z M80.9478262,15.7941755 L80.9478262,15.9191214 L79.7346008,15.9191214 L79.7346008,9.34321518 L77.6405064,9.34321518 L77.6405064,8.17247147 L83.0369228,8.17247147 L83.0369228,9.34321518 L80.9428284,9.34321518 L80.9428284,15.7941755 L80.9478262,15.7941755 L80.9478262,15.7941755 Z M75.5876442,15.9191214 L75.4626982,15.9191214 L75.4626982,8.17247147 L76.6746741,8.17247147 L76.6746741,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 Z M79.52844,32.0371512 C79.5370392,34.1970611 78.2424922,36.1490549 76.2494097,36.9814653 C74.2563272,37.8138756 71.9579919,37.362448 70.4278281,35.8380191 C68.8976642,34.3135903 68.4376096,32.0169663 69.2625301,30.0207722 C70.0874506,28.0245781 72.0345697,26.7227105 74.1944967,26.7231992 C75.6138156,26.6997406 76.9820945,27.252272 77.9871357,28.2547235 C78.9921768,29.2571749 79.5482366,30.6240237 79.52844,32.0433985 L79.52844,32.0371512 L79.52844,32.0371512 Z M74.1944967,28.8160441 C72.8711125,28.7215696 71.6246517,29.4463146 71.0520994,30.6431666 C70.4795471,31.8400187 70.6974742,33.2653004 71.6015059,34.2363811 C72.5055376,35.2074618 73.9116141,35.5266308 75.1463032,35.0410239 C76.3809922,34.555417 77.1929127,33.3639032 77.1931999,32.0371512 C77.2345906,31.2098229 76.9390613,30.401016 76.3740386,29.7952647 C75.809016,29.1895133 75.0227012,28.8384931 74.1944967,28.8222914 L74.1944967,28.8160441 L74.1944967,28.8160441 Z M69.8913576,15.7941755 L69.8913576,15.9191214 L68.6768828,15.9191214 L68.6768828,9.34321518 L66.5827884,9.34321518 L66.5827884,8.17247147 L71.985452,8.17247147 L71.985452,9.34321518 L69.8913576,9.34321518 L69.8913576,15.7941755 L69.8913576,15.7941755 L69.8913576,15.7941755 Z M60.351733,23.1060134 C57.0394569,23.1060134 54.3543266,25.7911438 54.3543266,29.1034199 C54.3543266,32.415696 57.0394569,35.1008263 60.351733,35.1008263 C61.911274,35.1640272 63.4248461,34.5649522 64.518681,33.4515395 C65.2354102,32.670278 65.6676971,31.6701194 65.7456504,30.6127671 L60.351733,30.6127671 L60.351733,28.3637397 L67.9397016,28.3637397 C68.0269136,28.8279579 68.0687593,29.2995593 68.0646475,29.7718808 C68.1283197,31.7175966 67.4272681,33.6108394 66.1117421,35.0458501 C64.6136241,36.6071212 62.5174198,37.4502439 60.3554814,37.3610988 C57.38266,37.3976772 54.6198728,35.8326549 53.1228026,33.2640394 C51.6257325,30.6954239 51.6257325,27.5201621 53.1228026,24.9515465 C54.6198728,22.382931 57.38266,20.8179088 60.3554814,20.8544871 C62.4700022,20.8223125 64.5085325,21.6423177 66.0117853,23.1297532 L64.4199737,24.7153175 C63.3290662,23.6635564 61.866938,23.0851715 60.351733,23.1060134 L60.351733,23.1060134 L60.351733,23.1060134 Z M61.202615,8.17247147 L65.8256158,8.17247147 L65.8256158,9.34321518 L62.4208382,9.34321518 L62.4208382,11.4672966 L65.4995068,11.4672966 L65.4995068,12.6167995 L62.4208382,12.6167995 L62.4208382,14.740881 L65.8356115,14.740881 L65.8356115,15.9191214 L61.2126107,15.9191214 L61.2126107,8.17247147 L61.202615,8.17247147 L61.202615,8.17247147 Z M59.1047722,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 C58.362729,15.7061967 57.3293999,16.1227078 56.2622515,16.0840501 C54.0306055,16.0840501 52.2214989,14.2749436 52.2214989,12.0432975 C52.2214989,9.81165151 54.0306055,8.00254496 56.2622515,8.00254496 C56.8081716,8.00062413 57.3486772,8.11076649 57.8503147,8.32615501 C58.319951,8.51770113 58.7377625,8.81736104 59.0697874,9.20077678 L59.1435055,9.28948842 L59.0622906,9.36945384 L58.3063675,10.1253769 L58.2189053,10.0216718 C57.7341734,9.45029146 57.0105226,9.13791092 56.2622515,9.17703705 C54.9671893,9.17687941 53.8320222,10.0429715 53.4901083,11.2920836 C53.1481944,12.5411957 53.6840183,13.8646806 54.798621,14.5241099 C55.9132237,15.1835391 57.3311944,15.0159721 58.261387,14.1149017 C58.5935495,13.7564153 58.7951569,13.296416 58.8336395,12.8092163 L56.1310582,12.8092163 L56.1310582,11.658464 L59.9931381,11.658464 L60.01188,11.7621691 C60.0471602,11.9559383 60.0651399,12.1524609 60.0656067,12.3494151 C60.111529,13.307955 59.7629594,14.2434942 59.1010239,14.9382956 L59.1010239,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 Z"
                            id="Text"
                            fill="#000000"
                            mask="url(#mask-2)"
                          />
                          <path
                            d="M15.9938175,41 C16.1330287,40.9450684 16.2683395,40.879997 16.3987426,40.8052703 L34.9704194,30.2486714 L30.5673087,25.6830961 L15.9938175,41 L15.9938175,41 L15.9938175,41 Z M35.7923475,29.7821131 L31.1991699,25.0191646 L35.7876859,20.1967398 L42.1067218,23.7761104 C43.6634906,24.6572404 42.8672013,25.7604152 42.1067218,26.1928298 L35.7923475,29.7821131 L35.7923475,29.7821131 L35.7923475,29.7821131 Z M34.9659696,19.7317234 L16.3987426,9.21169149 C16.1969509,9.10052181 15.9772574,9.02877096 15.7505657,9 L30.5630708,24.3594185 L34.9666053,19.7315032 L34.9666053,19.7315032 L34.9659696,19.7317234 Z M14.661654,9.18966324 L29.9312096,25.0231297 L14.7349685,40.9936118 C14.2914791,40.7015172 14.0537365,40.0684253 14.0497106,39.248754 C14.0420825,37.6796817 13.9213042,12.770796 13.9121928,10.9014786 C13.9077431,10.0064708 14.2217667,9.4597296 14.661654,9.18966324 L14.661654,9.18966324 Z"
                            id="Logo"
                            fill="#000000"
                            mask="url(#mask-2)"
                          />
                        </g>
                      </g>
                    </svg>
                  </a>
                </div>
              </div>
              <div className="banner-right">
                <div className="img" />
              </div>
            </div>
          </div>
        </section>
        <section id="reklam">
          <div className="container">
            {/* <h3 class="title anime">KNOWN FROM TV & MEDIA</h3> */}
            <div className="row">
              {/* <div class="logo-area">
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794bac651072fb96cffd44_800px-ZDF_logo.svg-p-500.png"
              alt="" class="logo" />
            <img src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794bafbe8e4400bb1f6603_unnamed.png"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acd751b23e8114bbf3745_maclife_logo.png"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794baea1d3ba37251e9897_ProSieben_Logo_2015.svg-p-500.png"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acb69fb0d7a1c8d774338_gruenderszene-logo-dark.svg"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794bafbe8e443dec1f6604_www.chip.de_ii_7_1_9_3_8_8_7_8_1e3d5abc96e4715a.jpeg"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acae4c06bf179b2830964_uni-stuttgart-logo-de.svg"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794badd6522b64fdb9346c_1200px-Maxi_Logo.svg-p-500.png"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e794bada1d3ba3ebe1e988a_chip-online-logo.png"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acc9462f35a4287dd5879_sat1-logo-p-500.jpeg"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acf26cee4fccfc1470d89_mittelbayerische-logo.jpg"
              alt="" class="logo" />
            <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604acfea0d69f2d544ec4b40_Stern-Logo-p-500.png"
              alt="" class="logo" />
          </div>
        </div> */}
            </div>
          </div>
        </section>
        <section id="app">
          <div className="container">
            <div className="row">
              <img
                className="app-top-img"
                src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5e99b2ad8f2dc785eab30829_ArriveSafe-Mockup-iPhone.png"
                alt=""
              />
            </div>
          </div>
          <div className="bg-element-two">
            {/*?xml version="1.0" encoding="UTF-8"?*/}
            <svg
              width="100%"
              height="60px"
              viewBox="0 0 1440 56"
              preserveAspectRatio="none"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <g
                id="Waves/wave-top"
                stroke="none"
                strokeWidth={1}
                fill="none"
                fillRule="evenodd"
              >
                <path
                  d="M1440,0.0100756918 L1440,37.3936566 L0,37.3936566 L0,0.802771506 C115.88527,0.307692596 229.081162,33.3936566 655.578314,33.3936566 C1082.07547,33.3936566 1250.37725,-0.67090601 1440,0.0100756918 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.5"
                  transform="translate(720.000000, 18.696828) scale(-1, -1) translate(-720.000000, -18.696828) "
                />
                <path
                  d="M1440,0.00505182912 L1440,55.3886327 L0,55.3886327 L0,5.79774764 C106.226448,5.34393264 313.375622,55.3886327 623.178314,55.3886327 C932.981007,55.3886327 1268.35502,-0.611367302 1440,0.00505182912 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.25"
                  transform="translate(720.000000, 27.694316) scale(-1, -1) translate(-720.000000, -27.694316) "
                />
                <path
                  d="M1440,0.0341359622 L1440,24.4177168 L0,24.4177168 L0,3.82683178 C95.7633114,3.41771683 278.134957,24.4177168 578.778314,24.4177168 C879.421672,24.4177168 1135.89541,-1.05797801 1440,0.0341359622 L1440,0.0341359622 Z"
                  id="base"
                  fill="#FFFFFF"
                  transform="translate(720.000000, 12.208858) scale(-1, -1) translate(-720.000000, -12.208858) "
                />
              </g>
            </svg>
          </div>
        </section>
        <section id="acardion">
          <div className="container">
            <div className="acardion-title">
              <span className="span"> Necə işləyir? </span>
              <h2 className="anime">Reminant tətbiqi necə işləyir?</h2>
            </div>
            <div className="row">
              <div className="left-item">
                <div className="left-img-item">
                  <img
                    className="img"
                    src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5ec126777b4d1122adf1219a_screen-start-de-p-500.png"
                    alt=""
                  />
                </div>
                <div className="left-img-item active">
                  <img
                    className="img"
                    src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5ec12677bfda0ea56ebefd59_screen-tracking-de-p-800.png"
                    alt=""
                  />
                </div>
                <div className="left-img-item">
                  <img
                    className="img"
                    src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5ec126777b4d1122adf1219a_screen-start-de-p-500.png"
                    alt=""
                  />
                </div>
              </div>
              <div className="right-item">
                <div className="right-flex ac">
                  <h4 className="right-title">1. Dizayn et</h4>
                  <p className="right-content">
                    Dizayn səhifəsində məhsulun üzərinə şəkil,simvol,mətn əlavə
                    etmək funksiyaları ilə geyiminizin üzərində arzuladığınız
                    görüntünü əldə edə bilərsiz.Hətta geymin rəngini belə dəyişə
                    bilərsiniz.Sonra isə dizaynınızı yadda saxlayın
                  </p>
                  <svg
                    className="icon"
                    style={{ fill: '#fff' }}
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                  >
                    <path d="M21.084 2.914c-1.178-1.179-3.234-1.179-4.412 0l-8.379 8.379a.999.999 0 0 0 0 1.414l3 3a.997.997 0 0 0 1.414 0l8.379-8.379a3.123 3.123 0 0 0-.002-4.414zm-1.412 3L12 13.586 10.414 12l7.672-7.672a1.146 1.146 0 0 1 1.586.002 1.123 1.123 0 0 1 0 1.584zM8 15c-1.265-.634-3.5 0-3.5 2 0 1.197.5 2-1.5 3 0 0 3.25 2.25 5.5 0 1.274-1.274 1.494-4-.5-5z"></path>
                  </svg>
                </div>
                <div className="right-flex active ">
                  <h4 className="right-title">2.Sifariş et</h4>
                  <p className="right-content">
                    Etdiyiniz dizaynlar "mənim dizaynlarım" səhifəsində yerləşir
                    orada sifariş etmək istədiyiniz dizayna daxil olub ölçüsünü
                    və sayını seçdikdən sonra "səbətə at" klikəyin.
                  </p>
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    width={24}
                    height={24}
                    viewBox="0 0 24 24"
                    style={{ fill: '#fff' }}
                  >
                    <path d="M21 9h-1.42l-3.712-6.496-1.736.992L17.277 9H6.723l3.146-5.504-1.737-.992L4.42 9H3a1.001 1.001 0 0 0-.965 1.263l2.799 10.264A2.005 2.005 0 0 0 6.764 22h10.473c.898 0 1.692-.605 1.93-1.475l2.799-10.263A.998.998 0 0 0 21 9zm-3.764 11v1-1H6.764L4.31 11h15.38l-2.454 9z"></path>
                    <path d="M9 13h2v5H9zm4 0h2v5h-2z" />
                  </svg>
                </div>
                <div className="right-flex ac">
                  <h4 className="right-title">3.Çatdırılma</h4>
                  <p className="right-content">
                    Lorem ipsum dolor sit amet consectetur explicabo voluptatum
                    natus labore dicta quo mollitia suscipit nobis praesentium
                    facilis ratione.
                  </p>
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    width={24}
                    height={24}
                    style={{ fill: '#fff' }}
                  >
                    <path d="M20.92 2.38A15.72 15.72 0 0 0 17.5 2a8.26 8.26 0 0 0-6 2.06Q9.89 5.67 8.31 7.27c-1.21-.13-4.08-.2-6 1.74a1 1 0 0 0 0 1.41l11.3 11.32a1 1 0 0 0 1.41 0c1.95-2 1.89-4.82 1.77-6l3.21-3.2c3.19-3.19 1.74-9.18 1.68-9.43a1 1 0 0 0-.76-.73zm-2.36 8.75L15 14.67a1 1 0 0 0-.27.9 6.81 6.81 0 0 1-.54 3.94L4.52 9.82a6.67 6.67 0 0 1 4-.5A1 1 0 0 0 9.39 9s1.4-1.45 3.51-3.56A6.61 6.61 0 0 1 17.5 4a14.51 14.51 0 0 1 2.33.2c.24 1.43.62 5.04-1.27 6.93z"></path>
                    <circle cx="15.73" cy="8.3" r={2} />
                    <path d="M5 16c-2 1-2 5-2 5a7.81 7.81 0 0 0 5-2z" />
                  </svg>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section id="emergency">
          <div className="container">
            <div className="row">
              <div className="emer-left">
                <span className="span"> Bildirişlər </span>
                <h2 className="anime">Sifarişiniz haqqında bildirişlər</h2>
                <p className="emer-content">
                  Bildirişlər səhifəsi vasitəsi ilə sifarişinizin harada
                  olduğunu və s rahatlıqla biləcəksiniz.
                </p>
                {/* <img
              src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/604ac67aaff100419424db65_ArriveSafeButton-p-500.png"
              alt="" /> */}
                {/* <p class="emer-content">
              To trigger emergencies even more intuitively and discreetly, you
              can optionally pair a KommGutHeim Bluetooth button. Your
              smartphone can stay in your pocket, just hold the button for 2
              seconds.
            </p> */}
                <a href="#" className="emer-btn">
                  {' '}
                  Yüklə{' '}
                </a>
              </div>
              <div className="emer-right">
                <img
                  src="https://uploads-ssl.webflow.com/5e794b4cf4f3316cdb57bab0/5ec1252037d7ce80073c81be_screen-emergency-de.png"
                  alt=""
                />
              </div>
            </div>
          </div>
        </section>
        <section id="card">
          <div className="container">
            {/* <span class="span">  </span> */}
            <h5 className="anime"> Funksionallıqlar </h5>
            <div className="row">
              <div className="card-grid">
                <div className="card-flex">
                  <div>
                    <svg
                      className="icon"
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M9 20h6v2H9zm7.906-6.288C17.936 12.506 19 11.259 19 9c0-3.859-3.141-7-7-7S5 5.141 5 9c0 2.285 1.067 3.528 2.101 4.73.358.418.729.851 1.084 1.349.144.206.38.996.591 1.921H8v2h8v-2h-.774c.213-.927.45-1.719.593-1.925.352-.503.726-.94 1.087-1.363zm-2.724.213c-.434.617-.796 2.075-1.006 3.075h-2.351c-.209-1.002-.572-2.463-1.011-3.08a20.502 20.502 0 0 0-1.196-1.492C7.644 11.294 7 10.544 7 9c0-2.757 2.243-5 5-5s5 2.243 5 5c0 1.521-.643 2.274-1.615 3.413-.373.438-.796.933-1.203 1.512z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Çeşitlilik</h6>
                    <p>Müxtəlif çeşitli geyim növləri</p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M21.316 4.055C19.556 3.478 15 1.985 15 2a3 3 0 1 1-6 0c0-.015-4.556 1.478-6.317 2.055A.992.992 0 0 0 2 5.003v3.716a1 1 0 0 0 1.242.97L6 9v12a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V9l2.758.689A1 1 0 0 0 22 8.719V5.003a.992.992 0 0 0-.684-.948z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Zövqünüz</h6>
                    <p>
                      Zövqünüz zövqümüzdür.Kapişon? ,T-shirt?,Sweatshirt? ... We
                      got your point !
                    </p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="m11 4-.5-1-.5 1-1 .125.834.708L9.5 6l1-.666 1 .666-.334-1.167.834-.708zm8.334 10.666L18.5 13l-.834 1.666-1.666.209 1.389 1.181L16.834 18l1.666-1.111L20.166 18l-.555-1.944L21 14.875zM6.667 6.333 6 5l-.667 1.333L4 6.5l1.111.944L4.667 9 6 8.111 7.333 9l-.444-1.556L8 6.5zM3.414 17c0 .534.208 1.036.586 1.414L5.586 20c.378.378.88.586 1.414.586s1.036-.208 1.414-.586L20 8.414c.378-.378.586-.88.586-1.414S20.378 5.964 20 5.586L18.414 4c-.756-.756-2.072-.756-2.828 0L4 15.586c-.378.378-.586.88-.586 1.414zM17 5.414 18.586 7 15 10.586 13.414 9 17 5.414z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Dizaynını yarat</h6>
                    <p>
                      Geyim mağazalarını saatlara gəzirsən və istədiyini tapa
                      bilmirsən? Bəs niyə istədiyini yaratmırsan?
                    </p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 18c-4.411 0-8-3.589-8-8s3.589-8 8-8 8 3.589 8 8-3.589 8-8 8z"></path>
                      <path d="M14.828 14.828a3.988 3.988 0 0 1-2.02 1.09 4.108 4.108 0 0 1-1.616 0 4.103 4.103 0 0 1-.749-.232 4.161 4.161 0 0 1-.679-.368 4.115 4.115 0 0 1-1.082-1.082l-1.658 1.117c.215.319.462.619.733.889a5.991 5.991 0 0 0 8.485.002c.272-.271.52-.571.734-.891l-1.658-1.117c-.143.211-.307.41-.49.592z"></path>
                      <circle cx="8.5" cy="10.5" r="1.5" />
                      <path d="M15.5 10c-2 0-2.5 2-2.5 2h5s-.501-2-2.5-2z" />
                    </svg>
                  </div>
                  <div>
                    <h6>Limitin fantaziyandır</h6>
                    <p>
                      Reminantda sənin limitin ancaq fantaziyandır.Dizayn
                      səhifəsindəki funksiyanallıqlarla istədiyini yarada
                      biləcəksən
                    </p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M13 10.551v-.678A4.005 4.005 0 0 0 16 6c0-2.206-1.794-4-4-4S8 3.794 8 6h2c0-1.103.897-2 2-2s2 .897 2 2-.897 2-2 2a1 1 0 0 0-1 1v1.551l-8.665 7.702A1.001 1.001 0 0 0 3 20h18a1.001 1.001 0 0 0 .664-1.748L13 10.551zM5.63 18 12 12.338 18.37 18H5.63z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Hazır geyimlər</h6>
                    <p>
                      Dizayn etməyə tənbəllik edirsənsə sənin üçündə aramızda
                      yer var
                    </p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M22 8a.76.76 0 0 0 0-.21v-.08a.77.77 0 0 0-.07-.16.35.35 0 0 0-.05-.08l-.1-.13-.08-.06-.12-.09-9-5a1 1 0 0 0-1 0l-9 5-.09.07-.11.08a.41.41 0 0 0-.07.11.39.39 0 0 0-.08.1.59.59 0 0 0-.06.14.3.3 0 0 0 0 .1A.76.76 0 0 0 2 8v8a1 1 0 0 0 .52.87l9 5a.75.75 0 0 0 .13.06h.1a1.06 1.06 0 0 0 .5 0h.1l.14-.06 9-5A1 1 0 0 0 22 16V8zm-10 3.87L5.06 8l2.76-1.52 6.83 3.9zm0-7.72L18.94 8 16.7 9.25 9.87 5.34zM4 9.7l7 3.92v5.68l-7-3.89zm9 9.6v-5.68l3-1.68V15l2-1v-3.18l2-1.11v5.7z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Çatdırılma</h6>
                    <p>
                      Bakı ərazisindəki dostlarımıza 2 gün ərzində çatdırılma
                      sözü !
                    </p>
                  </div>
                </div>
                <div className="card-flex">
                  <div>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={55}
                      height={55}
                      viewBox="0 0 24 24"
                      style={{
                        marginRight: '20px',
                        padding: '10px',
                        borderRadius: '50%',
                        background: '#fff',
                        fill: '#000',
                      }}
                    >
                      <path d="M16 12h2v4h-2z" />
                      <path d="M20 7V5c0-1.103-.897-2-2-2H5C3.346 3 2 4.346 2 6v12c0 2.201 1.794 3 3 3h15c1.103 0 2-.897 2-2V9c0-1.103-.897-2-2-2zM5 5h13v2H5a1.001 1.001 0 0 1 0-2zm15 14H5.012C4.55 18.988 4 18.805 4 18V8.815c.314.113.647.185 1 .185h15v10z"></path>
                    </svg>
                  </div>
                  <div>
                    <h6>Sərfəli qiymət,yüksək xidmət</h6>
                    <p>Hər növ materiallı geyim ilə istədiyini tap.</p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="img-top-abs">
            {/*?xml version="1.0" encoding="UTF-8"?*/}
            <svg
              width="100%"
              height="60px"
              viewBox="0 0 1440 56"
              preserveAspectRatio="none"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <g
                id="Waves/wave-top"
                stroke="none"
                strokeWidth={1}
                fill="none"
                fillRule="evenodd"
              >
                <path
                  d="M1440,0.0100756918 L1440,37.3936566 L0,37.3936566 L0,0.802771506 C115.88527,0.307692596 229.081162,33.3936566 655.578314,33.3936566 C1082.07547,33.3936566 1250.37725,-0.67090601 1440,0.0100756918 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.5"
                  transform="translate(720.000000, 18.696828) scale(-1, -1) translate(-720.000000, -18.696828) "
                />
                <path
                  d="M1440,0.00505182912 L1440,55.3886327 L0,55.3886327 L0,5.79774764 C106.226448,5.34393264 313.375622,55.3886327 623.178314,55.3886327 C932.981007,55.3886327 1268.35502,-0.611367302 1440,0.00505182912 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.25"
                  transform="translate(720.000000, 27.694316) scale(-1, -1) translate(-720.000000, -27.694316) "
                />
                <path
                  d="M1440,0.0341359622 L1440,24.4177168 L0,24.4177168 L0,3.82683178 C95.7633114,3.41771683 278.134957,24.4177168 578.778314,24.4177168 C879.421672,24.4177168 1135.89541,-1.05797801 1440,0.0341359622 L1440,0.0341359622 Z"
                  id="base"
                  fill="#FFFFFF"
                  transform="translate(720.000000, 12.208858) scale(-1, -1) translate(-720.000000, -12.208858) "
                />
              </g>
            </svg>
          </div>
          <div className="img-bottom-abs">
            {/*?xml version="1.0" encoding="UTF-8"?*/}
            <svg
              width="100%"
              height="56px"
              viewBox="0 0 1440 56"
              preserveAspectRatio="none"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <g
                id="Waves/wave-bottom"
                stroke="none"
                strokeWidth={1}
                fill="none"
                fillRule="evenodd"
              >
                <path
                  d="M1440,18.6164191 L1440,56 L0,56 L0,19.4091149 C115.88527,18.914036 229.081162,52 655.578314,52 C1082.07547,52 1250.37725,17.9354374 1440,18.6164191 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.5"
                />
                <path
                  d="M1440,0.616419131 L1440,56 L0,56 L0,6.40911495 C106.226448,5.95529994 313.375622,56 623.178314,56 C932.981007,56 1268.35502,-9.23705556e-14 1440,0.616419131 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.25"
                />
                <path
                  d="M1440,31.6164191 L1440,56 L0,56 L0,35.4091149 C95.7633114,35 278.134957,56 578.778314,56 C879.421672,56 1135.89541,30.5243052 1440,31.6164191 L1440,31.6164191 Z"
                  id="base"
                  fill="#FFFFFF"
                />
              </g>
            </svg>
          </div>
        </section>
        <section id="comment">
          <div className="coment-container">
            <span className="span"> İstifadəçi rəyləri </span>
            <h2 className="anime">
              İstifadəçilərmizin rəylərinə nəzər yetirin.
            </h2>
            <div className="row">
              <div className="coment">
                <p>
                  "Proqram əla işləyir. Dostumun ad günü idi və mən ona özünün
                  cool şəkli olan bir köynək hədiyyə etdim. Çox bəyəndi :)"
                </p>
                <div className="coment-flex">
                  <div>
                    <svg
                      className="icoN"
                      xmlns="http://www.w3.org/2000/svg"
                      width={25}
                      height={25}
                      viewBox="0 0 25 25"
                      style={{
                        padding: '9px 3px 5px 5px',
                        fill: 'rgba(255, 255, 255, 1)',
                      }}
                    >
                      <path d="M20 2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h3v3.766L13.277 18H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14h-7.277L9 18.234V16H4V4h16v12z"></path>
                      <circle cx={15} cy={10} r={2} />
                      <circle cx={9} cy={10} r={2} />
                    </svg>
                  </div>
                  <div className="flex-div">
                    <p className="name">Əli</p>
                    <p className="app">PLAY STORE</p>
                  </div>
                  <div className="star">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                  </div>
                </div>
              </div>
              <div className="coment">
                <p>"Çatdırılmanın bu qədər tez olacağını gözləmirdim.Super"</p>
                <div className="coment-flex">
                  <div>
                    <svg
                      className="icoN"
                      xmlns="http://www.w3.org/2000/svg"
                      width={25}
                      height={25}
                      viewBox="0 0 25 25"
                      style={{
                        padding: '9px 3px 5px 5px',
                        fill: 'rgba(255, 255, 255, 1)',
                      }}
                    >
                      <path d="M20 2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h3v3.766L13.277 18H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14h-7.277L9 18.234V16H4V4h16v12z"></path>
                      <circle cx={15} cy={10} r={2} />
                      <circle cx={9} cy={10} r={2} />
                    </svg>
                  </div>
                  <div className="flex-div">
                    <p className="name">Xəyalə</p>
                    <p className="app">PLAY STORE</p>
                  </div>
                  <div className="star">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                  </div>
                </div>
              </div>
              <div className="coment">
                <p>
                  "Paltarların qiymətləri verilən xidmətə görə sərfəlidir.Mənim
                  mağazamın müştəriləri sizin tətbiqlə hazırladığım kapişonlar
                  xoşlayır.Sizin proqramı istifadə edəndən sonra qazancım 2 qat
                  artıb."
                </p>
                <div className="coment-flex">
                  <div>
                    <svg
                      className="icoN"
                      xmlns="http://www.w3.org/2000/svg"
                      width={25}
                      height={25}
                      viewBox="0 0 25 25"
                      style={{
                        padding: '9px 3px 5px 5px',
                        fill: 'rgba(255, 255, 255, 1)',
                      }}
                    >
                      <path d="M20 2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h3v3.766L13.277 18H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14h-7.277L9 18.234V16H4V4h16v12z"></path>
                      <circle cx={15} cy={10} r={2} />
                      <circle cx={9} cy={10} r={2} />
                    </svg>
                  </div>
                  <div className="flex-div">
                    <p className="name">Həsən</p>
                    <p className="app">PLAY STORE</p>
                  </div>
                  <div className="star">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                  </div>
                </div>
              </div>
              <div className="coment">
                <p>
                  "Restaranımdakı işçilərim özümün dizayn etdiyim iş
                  geyimlərindən isitfadə edir.Çox yaraşdı."
                </p>
                <div className="coment-flex">
                  <div>
                    <svg
                      className="icoN"
                      xmlns="http://www.w3.org/2000/svg"
                      width={25}
                      height={25}
                      viewBox="0 0 25 25"
                      style={{
                        padding: '9px 3px 5px 5px',
                        fill: 'rgba(255, 255, 255, 1)',
                      }}
                    >
                      <path d="M20 2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h3v3.766L13.277 18H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14h-7.277L9 18.234V16H4V4h16v12z"></path>
                      <circle cx={15} cy={10} r={2} />
                      <circle cx={9} cy={10} r={2} />
                    </svg>
                  </div>
                  <div className="flex-div">
                    <p className="name">Rüstəm</p>
                    <p className="app">PLAY STORE</p>
                  </div>
                  <div className="star">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={16}
                      viewBox="0 0 24 24"
                      style={{ fill: 'rgba(255, 165, 0, 1)' }}
                    >
                      <path d="M21.947 9.179a1.001 1.001 0 0 0-.868-.676l-5.701-.453-2.467-5.461a.998.998 0 0 0-1.822-.001L8.622 8.05l-5.701.453a1 1 0 0 0-.619 1.713l4.213 4.107-1.49 6.452a1 1 0 0 0 1.53 1.057L12 18.202l5.445 3.63a1.001 1.001 0 0 0 1.517-1.106l-1.829-6.4 4.536-4.082c.297-.268.406-.686.278-1.065z"></path>
                    </svg>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section id="last">
          <div className="container">
            <div className="row">
              <div className="last-item">
                <div>
                  <p className="p1">Yüklə</p>
                  <p className="p2">
                    Pulsuz Reminant Tətbiqini endirin və özünüzə məxsus geyim
                    tərzinizi yaradın!
                  </p>
                </div>
                <div className="btn-flex">
                  <a className="android" href="./install.html">
                    {/*?xml version="1.0" encoding="UTF-8"?*/}
                    <svg
                      width="170px"
                      height="50px"
                      viewBox="0 0 170 50"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlnsXlink="http://www.w3.org/1999/xlink"
                    >
                      <title>Store Badges/google-play-white</title>
                      <desc>Created with Sketch.</desc>
                      <defs>
                        <path
                          d="M5.95503704,50 C2.67214815,50 0,47.34 0,44.08375 L0,5.9075 C0,2.65 2.67214815,0 5.95503704,0 L163.951778,0 C167.233407,0 170,2.65 170,5.9075 L170,44.08375 C170,47.34 167.233407,50 163.951778,50 L5.95503704,50 Z"
                          id="path-1"
                        />
                      </defs>
                      <g
                        id="Store-Badges/google-play-white"
                        stroke="none"
                        strokeWidth={1}
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g id="Google-Play-Badge">
                          <mask id="mask-2" fill="white">
                            <use xlinkHref="#path-1" />
                          </mask>
                          <use id="Mask" fill="#FFFFFF" xlinkHref="#path-1" />
                          <path
                            d="M151.49107,41.7716915 L149.059621,41.7716915 L151.433595,36.5064684 L147.271644,27.0393125 L149.783058,27.0393125 L152.556859,33.8113839 L152.631826,33.8113839 L155.304421,27.0393125 L157.897049,27.0393125 L151.49107,41.7716915 L151.49107,41.7716915 Z M144.860187,35.8030226 L144.780222,35.8030226 C144.140945,36.8162582 143.001228,37.4016736 141.805258,37.3311118 C140.832387,37.4033509 139.8717,37.0787381 139.142032,36.4312174 C138.412365,35.7836967 137.975854,34.8684087 137.931933,33.8938482 C137.931933,31.6035887 140.203451,30.3778687 142.429988,30.3778687 C143.277645,30.3757657 144.112755,30.5826121 144.861437,30.9801083 L144.861437,30.8201774 C144.735847,29.5901136 143.640843,28.6921208 142.409997,28.8097968 C141.535301,28.7344576 140.69901,29.1842396 140.279668,29.9555514 L138.210563,29.0909253 C138.997314,27.5369338 140.631245,26.598 142.370014,26.7007089 C145.042608,26.7007089 147.117961,28.24754 147.117961,31.020091 L147.117961,37.0087512 L144.868933,37.0087512 L144.868933,35.8030226 L144.860187,35.8030226 L144.860187,35.8030226 Z M142.729858,32.2670517 C141.584104,32.2670517 140.258427,32.7293518 140.258427,33.9350804 C140.410733,34.7980563 141.220125,35.3846444 142.087636,35.2607571 C143.499384,35.2450794 144.683015,34.1899998 144.860187,32.7893259 C144.209732,32.4274207 143.473946,32.2470344 142.729858,32.2670517 L142.729858,32.2670517 L142.729858,32.2670517 Z M134.30975,21.4154945 L136.638743,21.4154945 L136.638743,37.0349898 L134.307251,37.0349898 L134.307251,21.416744 L134.30975,21.4154945 L134.30975,21.4154945 Z M127.900022,31.1175489 L124.64393,31.1175489 L124.64393,37.0349898 L122.313687,37.0349898 L122.313687,21.416744 L127.900022,21.416744 C129.697254,21.316782 131.402439,22.2187799 132.33116,23.7606992 C133.25988,25.3026185 133.25988,27.2316743 132.33116,28.7735936 C131.402439,30.3155129 129.697254,31.2175108 127.900022,31.1175489 L127.900022,31.1175489 L127.900022,31.1175489 Z M127.959996,23.5895543 L124.64393,23.5895543 L124.64393,28.943489 L127.959996,28.943489 C128.930171,28.9644502 129.835756,28.4588571 130.326995,27.6219804 C130.818235,26.7851036 130.818235,25.7479398 130.326995,24.911063 C129.835756,24.0741862 128.930171,23.5685932 127.959996,23.5895543 L127.959996,23.5895543 L127.959996,23.5895543 Z M112.675356,35.263256 C113.745483,35.2687796 114.739492,34.7105394 115.291724,33.7938915 L117.103441,35.002119 C116.117632,36.48215 114.453617,37.3672143 112.675356,37.3573504 C111.263495,37.3777077 109.904053,36.8230642 108.90939,35.8208607 C107.914727,34.8186572 107.370367,33.4550655 107.401386,32.0433985 C107.328974,30.6655143 107.823612,29.3180221 108.770291,28.3142259 C109.71697,27.3104297 111.033209,26.7377885 112.412969,26.7294465 C114.489527,26.787608 116.307745,28.1389117 116.962252,30.1104844 L117.203397,30.7139734 L110.09772,33.653952 C110.545987,34.6678995 111.567551,35.3056956 112.675356,35.263256 L112.675356,35.263256 L112.675356,35.263256 Z M114.465831,29.9093213 C114.093761,29.1722381 113.316738,28.7293498 112.492934,28.7848076 C111.710817,28.8200937 110.975771,29.1685109 110.453317,29.7516007 C109.930863,30.3346906 109.664924,31.1034324 109.715386,31.8847171 L114.465831,29.9093213 L114.465831,29.9093213 Z M103.937884,21.4129956 L106.276873,21.4129956 L106.276873,37.0349898 L103.941632,37.0349898 L103.941632,21.416744 L103.937884,21.4129956 L103.937884,21.4129956 Z M97.280763,42.1277875 C95.2175856,42.1384522 93.3542191,40.8963727 92.5703,38.9878953 L94.6031709,38.1420111 C95.0289406,39.2576525 96.0869789,40.005142 97.280763,40.0336931 C99.0300065,40.0336931 100.119535,38.9466631 100.119535,36.9100439 L100.119535,36.1453746 L100.029574,36.1453746 C99.3199718,36.9397219 98.2968057,37.3816163 97.2320341,37.3536021 C94.3867898,37.2301726 92.1436087,34.8875703 92.1436087,32.0396501 C92.1436087,29.1917298 94.3867898,26.8491276 97.2320341,26.7256981 C98.2919159,26.700318 99.3113398,27.132856 100.029574,27.9126848 L100.110789,27.9126848 L100.110789,27.0468092 L102.324832,27.0468092 L102.324832,36.5889327 C102.32858,40.5172339 100.013331,42.1277875 97.280763,42.1277875 L97.280763,42.1277875 L97.280763,42.1277875 Z M97.4419433,28.8222914 C95.7702029,28.9659548 94.4863479,30.3648718 94.4863479,32.0427737 C94.4863479,33.7206757 95.7702029,35.1195926 97.4419433,35.263256 C99.0569943,35.0481199 100.262855,33.6699002 100.261591,32.0405839 C100.260326,30.4112676 99.0523284,29.0349209 97.4369455,28.8222914 L97.4419433,28.8222914 L97.4419433,28.8222914 Z M99.835908,15.9191214 L96.1874858,10.0679018 L96.2187223,11.1224458 L96.2187223,15.9191214 L95.0054969,15.9191214 L95.0054969,8.17247147 L96.3724058,8.17247147 L99.852151,13.7413132 L99.822164,12.6855198 L99.822164,8.17247147 L101.029142,8.17247147 L101.029142,15.9191214 L99.8296607,15.9191214 L99.835908,15.9191214 L99.835908,15.9191214 Z M89.9251939,16.086549 C88.8563149,16.1112835 87.8263874,15.6849815 87.087671,14.9120569 C85.5682608,13.3032575 85.5682608,10.7883354 87.087671,9.17953597 C87.8381682,8.42453548 88.8587674,8 89.9233197,8 C90.9878721,8 92.0084713,8.42453548 92.7589684,9.17953597 C94.2815027,10.7851615 94.2815027,13.3014335 92.7589684,14.9070591 C92.0204707,15.6816261 90.9901475,16.1097181 89.9201961,16.086549 L89.9251939,16.086549 L89.9251939,16.086549 Z M87.9785358,9.98168907 C86.9193832,11.1520421 86.9193832,12.9345529 87.9785358,14.104906 C89.055542,15.1777887 90.7973447,15.1777887 91.874351,14.104906 C92.9323135,12.9331719 92.9296121,11.1502118 91.8681037,9.98168907 C90.7965232,8.90895059 89.055114,8.90895059 87.9785358,9.98168907 L87.9785358,9.98168907 Z M91.1696557,32.0433985 C91.1777483,34.202578 89.8833182,36.1536544 87.8908144,36.98555 C85.8983107,37.8174457 83.6008153,37.3660439 82.0711703,35.8421319 C80.5415253,34.31822 80.0814996,32.0224358 80.9059077,30.0268224 C81.7303157,28.0312089 83.6765178,26.7294617 85.8357124,26.7294465 C87.2530734,26.7073934 88.6190698,27.2596784 89.6228278,28.2606122 C90.6265858,29.261546 91.1827174,30.6259809 91.1646579,32.0433985 L91.1696557,32.0433985 L91.1696557,32.0433985 Z M85.8357124,28.8222914 C84.5123283,28.7278169 83.2658674,29.4525619 82.6933151,30.6494139 C82.1207628,31.846266 82.33869,33.2715477 83.2427217,34.2426284 C84.1467534,35.2137091 85.5528299,35.5328781 86.7875189,35.0472712 C88.0222079,34.5616643 88.8341284,33.3701505 88.8344156,32.0433985 C88.8775728,31.2141675 88.5821524,30.4029572 88.0158558,29.7956724 C87.4495592,29.1883876 86.660936,28.8370919 85.8307146,28.8222914 L85.8357124,28.8222914 L85.8357124,28.8222914 Z M80.9478262,15.7941755 L80.9478262,15.9191214 L79.7346008,15.9191214 L79.7346008,9.34321518 L77.6405064,9.34321518 L77.6405064,8.17247147 L83.0369228,8.17247147 L83.0369228,9.34321518 L80.9428284,9.34321518 L80.9428284,15.7941755 L80.9478262,15.7941755 L80.9478262,15.7941755 Z M75.5876442,15.9191214 L75.4626982,15.9191214 L75.4626982,8.17247147 L76.6746741,8.17247147 L76.6746741,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 Z M79.52844,32.0371512 C79.5370392,34.1970611 78.2424922,36.1490549 76.2494097,36.9814653 C74.2563272,37.8138756 71.9579919,37.362448 70.4278281,35.8380191 C68.8976642,34.3135903 68.4376096,32.0169663 69.2625301,30.0207722 C70.0874506,28.0245781 72.0345697,26.7227105 74.1944967,26.7231992 C75.6138156,26.6997406 76.9820945,27.252272 77.9871357,28.2547235 C78.9921768,29.2571749 79.5482366,30.6240237 79.52844,32.0433985 L79.52844,32.0371512 L79.52844,32.0371512 Z M74.1944967,28.8160441 C72.8711125,28.7215696 71.6246517,29.4463146 71.0520994,30.6431666 C70.4795471,31.8400187 70.6974742,33.2653004 71.6015059,34.2363811 C72.5055376,35.2074618 73.9116141,35.5266308 75.1463032,35.0410239 C76.3809922,34.555417 77.1929127,33.3639032 77.1931999,32.0371512 C77.2345906,31.2098229 76.9390613,30.401016 76.3740386,29.7952647 C75.809016,29.1895133 75.0227012,28.8384931 74.1944967,28.8222914 L74.1944967,28.8160441 L74.1944967,28.8160441 Z M69.8913576,15.7941755 L69.8913576,15.9191214 L68.6768828,15.9191214 L68.6768828,9.34321518 L66.5827884,9.34321518 L66.5827884,8.17247147 L71.985452,8.17247147 L71.985452,9.34321518 L69.8913576,9.34321518 L69.8913576,15.7941755 L69.8913576,15.7941755 L69.8913576,15.7941755 Z M60.351733,23.1060134 C57.0394569,23.1060134 54.3543266,25.7911438 54.3543266,29.1034199 C54.3543266,32.415696 57.0394569,35.1008263 60.351733,35.1008263 C61.911274,35.1640272 63.4248461,34.5649522 64.518681,33.4515395 C65.2354102,32.670278 65.6676971,31.6701194 65.7456504,30.6127671 L60.351733,30.6127671 L60.351733,28.3637397 L67.9397016,28.3637397 C68.0269136,28.8279579 68.0687593,29.2995593 68.0646475,29.7718808 C68.1283197,31.7175966 67.4272681,33.6108394 66.1117421,35.0458501 C64.6136241,36.6071212 62.5174198,37.4502439 60.3554814,37.3610988 C57.38266,37.3976772 54.6198728,35.8326549 53.1228026,33.2640394 C51.6257325,30.6954239 51.6257325,27.5201621 53.1228026,24.9515465 C54.6198728,22.382931 57.38266,20.8179088 60.3554814,20.8544871 C62.4700022,20.8223125 64.5085325,21.6423177 66.0117853,23.1297532 L64.4199737,24.7153175 C63.3290662,23.6635564 61.866938,23.0851715 60.351733,23.1060134 L60.351733,23.1060134 L60.351733,23.1060134 Z M61.202615,8.17247147 L65.8256158,8.17247147 L65.8256158,9.34321518 L62.4208382,9.34321518 L62.4208382,11.4672966 L65.4995068,11.4672966 L65.4995068,12.6167995 L62.4208382,12.6167995 L62.4208382,14.740881 L65.8356115,14.740881 L65.8356115,15.9191214 L61.2126107,15.9191214 L61.2126107,8.17247147 L61.202615,8.17247147 L61.202615,8.17247147 Z M59.1047722,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 C58.362729,15.7061967 57.3293999,16.1227078 56.2622515,16.0840501 C54.0306055,16.0840501 52.2214989,14.2749436 52.2214989,12.0432975 C52.2214989,9.81165151 54.0306055,8.00254496 56.2622515,8.00254496 C56.8081716,8.00062413 57.3486772,8.11076649 57.8503147,8.32615501 C58.319951,8.51770113 58.7377625,8.81736104 59.0697874,9.20077678 L59.1435055,9.28948842 L59.0622906,9.36945384 L58.3063675,10.1253769 L58.2189053,10.0216718 C57.7341734,9.45029146 57.0105226,9.13791092 56.2622515,9.17703705 C54.9671893,9.17687941 53.8320222,10.0429715 53.4901083,11.2920836 C53.1481944,12.5411957 53.6840183,13.8646806 54.798621,14.5241099 C55.9132237,15.1835391 57.3311944,15.0159721 58.261387,14.1149017 C58.5935495,13.7564153 58.7951569,13.296416 58.8336395,12.8092163 L56.1310582,12.8092163 L56.1310582,11.658464 L59.9931381,11.658464 L60.01188,11.7621691 C60.0471602,11.9559383 60.0651399,12.1524609 60.0656067,12.3494151 C60.111529,13.307955 59.7629594,14.2434942 59.1010239,14.9382956 L59.1010239,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 Z"
                            id="Text"
                            fill="#000000"
                            mask="url(#mask-2)"
                          />
                          <path
                            d="M15.9938175,41 C16.1330287,40.9450684 16.2683395,40.879997 16.3987426,40.8052703 L34.9704194,30.2486714 L30.5673087,25.6830961 L15.9938175,41 L15.9938175,41 L15.9938175,41 Z M35.7923475,29.7821131 L31.1991699,25.0191646 L35.7876859,20.1967398 L42.1067218,23.7761104 C43.6634906,24.6572404 42.8672013,25.7604152 42.1067218,26.1928298 L35.7923475,29.7821131 L35.7923475,29.7821131 L35.7923475,29.7821131 Z M34.9659696,19.7317234 L16.3987426,9.21169149 C16.1969509,9.10052181 15.9772574,9.02877096 15.7505657,9 L30.5630708,24.3594185 L34.9666053,19.7315032 L34.9666053,19.7315032 L34.9659696,19.7317234 Z M14.661654,9.18966324 L29.9312096,25.0231297 L14.7349685,40.9936118 C14.2914791,40.7015172 14.0537365,40.0684253 14.0497106,39.248754 C14.0420825,37.6796817 13.9213042,12.770796 13.9121928,10.9014786 C13.9077431,10.0064708 14.2217667,9.4597296 14.661654,9.18966324 L14.661654,9.18966324 Z"
                            id="Logo"
                            fill="#000000"
                            mask="url(#mask-2)"
                          />
                        </g>
                      </g>
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </div>
          <div className="img-abs-last">
            {/*?xml version="1.0" encoding="UTF-8"?*/}
            <svg
              width="100%"
              height="56px"
              viewBox="0 0 1440 56"
              preserveAspectRatio="none"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <g
                id="Waves/wave-top"
                stroke="none"
                strokeWidth={1}
                fill="none"
                fillRule="evenodd"
              >
                <path
                  d="M1440,0.0100756918 L1440,37.3936566 L0,37.3936566 L0,0.802771506 C115.88527,0.307692596 229.081162,33.3936566 655.578314,33.3936566 C1082.07547,33.3936566 1250.37725,-0.67090601 1440,0.0100756918 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.5"
                  transform="translate(720.000000, 18.696828) scale(-1, -1) translate(-720.000000, -18.696828) "
                />
                <path
                  d="M1440,0.00505182912 L1440,55.3886327 L0,55.3886327 L0,5.79774764 C106.226448,5.34393264 313.375622,55.3886327 623.178314,55.3886327 C932.981007,55.3886327 1268.35502,-0.611367302 1440,0.00505182912 Z"
                  id="wave"
                  fill="#FFFFFF"
                  opacity="0.25"
                  transform="translate(720.000000, 27.694316) scale(-1, -1) translate(-720.000000, -27.694316) "
                />
                <path
                  d="M1440,0.0341359622 L1440,24.4177168 L0,24.4177168 L0,3.82683178 C95.7633114,3.41771683 278.134957,24.4177168 578.778314,24.4177168 C879.421672,24.4177168 1135.89541,-1.05797801 1440,0.0341359622 L1440,0.0341359622 Z"
                  id="base"
                  fill="#FFFFFF"
                  transform="translate(720.000000, 12.208858) scale(-1, -1) translate(-720.000000, -12.208858) "
                />
              </g>
            </svg>
          </div>
        </section>
      </main>
      <footer id="footer">
        <div className="container">
          <div className="row1">
            <div className="left">
              <a href="/" className="footer-logo">
                <img src="/assets/images/logos/logo.png" />
                <span>Reminant</span>
              </a>
            </div>
            <div className="right">
              <div className="btn-flex">
                <a className="android" href="./install.html">
                  {/*?xml version="1.0" encoding="UTF-8"?*/}
                  <svg
                    width="170px"
                    height="50px"
                    viewBox="0 0 170 50"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlnsXlink="http://www.w3.org/1999/xlink"
                  >
                    {/* Generator: Sketch 55.2 (78181) - https://sketchapp.com */}
                    <title>Store Badges/google-play</title>
                    <desc>Created with Sketch.</desc>
                    <defs>
                      <path
                        d="M5.95503704,50 C2.67214815,50 0,47.34 0,44.08375 L0,5.9075 C0,2.65 2.67214815,0 5.95503704,0 L163.951778,0 C167.233407,0 170,2.65 170,5.9075 L170,44.08375 C170,47.34 167.233407,50 163.951778,50 L5.95503704,50 Z"
                        id="path-1"
                      />
                    </defs>
                    <g
                      id="Store-Badges/google-play"
                      stroke="none"
                      strokeWidth={1}
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g id="Google-Play-Badge">
                        <mask id="mask-2" fill="white">
                          <use xlinkHref="#path-1" />
                        </mask>
                        <use id="Mask" fill="#000000" xlinkHref="#path-1" />
                        <path
                          d="M151.49107,41.7716915 L149.059621,41.7716915 L151.433595,36.5064684 L147.271644,27.0393125 L149.783058,27.0393125 L152.556859,33.8113839 L152.631826,33.8113839 L155.304421,27.0393125 L157.897049,27.0393125 L151.49107,41.7716915 L151.49107,41.7716915 Z M144.860187,35.8030226 L144.780222,35.8030226 C144.140945,36.8162582 143.001228,37.4016736 141.805258,37.3311118 C140.832387,37.4033509 139.8717,37.0787381 139.142032,36.4312174 C138.412365,35.7836967 137.975854,34.8684087 137.931933,33.8938482 C137.931933,31.6035887 140.203451,30.3778687 142.429988,30.3778687 C143.277645,30.3757657 144.112755,30.5826121 144.861437,30.9801083 L144.861437,30.8201774 C144.735847,29.5901136 143.640843,28.6921208 142.409997,28.8097968 C141.535301,28.7344576 140.69901,29.1842396 140.279668,29.9555514 L138.210563,29.0909253 C138.997314,27.5369338 140.631245,26.598 142.370014,26.7007089 C145.042608,26.7007089 147.117961,28.24754 147.117961,31.020091 L147.117961,37.0087512 L144.868933,37.0087512 L144.868933,35.8030226 L144.860187,35.8030226 L144.860187,35.8030226 Z M142.729858,32.2670517 C141.584104,32.2670517 140.258427,32.7293518 140.258427,33.9350804 C140.410733,34.7980563 141.220125,35.3846444 142.087636,35.2607571 C143.499384,35.2450794 144.683015,34.1899998 144.860187,32.7893259 C144.209732,32.4274207 143.473946,32.2470344 142.729858,32.2670517 L142.729858,32.2670517 L142.729858,32.2670517 Z M134.30975,21.4154945 L136.638743,21.4154945 L136.638743,37.0349898 L134.307251,37.0349898 L134.307251,21.416744 L134.30975,21.4154945 L134.30975,21.4154945 Z M127.900022,31.1175489 L124.64393,31.1175489 L124.64393,37.0349898 L122.313687,37.0349898 L122.313687,21.416744 L127.900022,21.416744 C129.697254,21.316782 131.402439,22.2187799 132.33116,23.7606992 C133.25988,25.3026185 133.25988,27.2316743 132.33116,28.7735936 C131.402439,30.3155129 129.697254,31.2175108 127.900022,31.1175489 L127.900022,31.1175489 L127.900022,31.1175489 Z M127.959996,23.5895543 L124.64393,23.5895543 L124.64393,28.943489 L127.959996,28.943489 C128.930171,28.9644502 129.835756,28.4588571 130.326995,27.6219804 C130.818235,26.7851036 130.818235,25.7479398 130.326995,24.911063 C129.835756,24.0741862 128.930171,23.5685932 127.959996,23.5895543 L127.959996,23.5895543 L127.959996,23.5895543 Z M112.675356,35.263256 C113.745483,35.2687796 114.739492,34.7105394 115.291724,33.7938915 L117.103441,35.002119 C116.117632,36.48215 114.453617,37.3672143 112.675356,37.3573504 C111.263495,37.3777077 109.904053,36.8230642 108.90939,35.8208607 C107.914727,34.8186572 107.370367,33.4550655 107.401386,32.0433985 C107.328974,30.6655143 107.823612,29.3180221 108.770291,28.3142259 C109.71697,27.3104297 111.033209,26.7377885 112.412969,26.7294465 C114.489527,26.787608 116.307745,28.1389117 116.962252,30.1104844 L117.203397,30.7139734 L110.09772,33.653952 C110.545987,34.6678995 111.567551,35.3056956 112.675356,35.263256 L112.675356,35.263256 L112.675356,35.263256 Z M114.465831,29.9093213 C114.093761,29.1722381 113.316738,28.7293498 112.492934,28.7848076 C111.710817,28.8200937 110.975771,29.1685109 110.453317,29.7516007 C109.930863,30.3346906 109.664924,31.1034324 109.715386,31.8847171 L114.465831,29.9093213 L114.465831,29.9093213 Z M103.937884,21.4129956 L106.276873,21.4129956 L106.276873,37.0349898 L103.941632,37.0349898 L103.941632,21.416744 L103.937884,21.4129956 L103.937884,21.4129956 Z M97.280763,42.1277875 C95.2175856,42.1384522 93.3542191,40.8963727 92.5703,38.9878953 L94.6031709,38.1420111 C95.0289406,39.2576525 96.0869789,40.005142 97.280763,40.0336931 C99.0300065,40.0336931 100.119535,38.9466631 100.119535,36.9100439 L100.119535,36.1453746 L100.029574,36.1453746 C99.3199718,36.9397219 98.2968057,37.3816163 97.2320341,37.3536021 C94.3867898,37.2301726 92.1436087,34.8875703 92.1436087,32.0396501 C92.1436087,29.1917298 94.3867898,26.8491276 97.2320341,26.7256981 C98.2919159,26.700318 99.3113398,27.132856 100.029574,27.9126848 L100.110789,27.9126848 L100.110789,27.0468092 L102.324832,27.0468092 L102.324832,36.5889327 C102.32858,40.5172339 100.013331,42.1277875 97.280763,42.1277875 L97.280763,42.1277875 L97.280763,42.1277875 Z M97.4419433,28.8222914 C95.7702029,28.9659548 94.4863479,30.3648718 94.4863479,32.0427737 C94.4863479,33.7206757 95.7702029,35.1195926 97.4419433,35.263256 C99.0569943,35.0481199 100.262855,33.6699002 100.261591,32.0405839 C100.260326,30.4112676 99.0523284,29.0349209 97.4369455,28.8222914 L97.4419433,28.8222914 L97.4419433,28.8222914 Z M99.835908,15.9191214 L96.1874858,10.0679018 L96.2187223,11.1224458 L96.2187223,15.9191214 L95.0054969,15.9191214 L95.0054969,8.17247147 L96.3724058,8.17247147 L99.852151,13.7413132 L99.822164,12.6855198 L99.822164,8.17247147 L101.029142,8.17247147 L101.029142,15.9191214 L99.8296607,15.9191214 L99.835908,15.9191214 L99.835908,15.9191214 Z M89.9251939,16.086549 C88.8563149,16.1112835 87.8263874,15.6849815 87.087671,14.9120569 C85.5682608,13.3032575 85.5682608,10.7883354 87.087671,9.17953597 C87.8381682,8.42453548 88.8587674,8 89.9233197,8 C90.9878721,8 92.0084713,8.42453548 92.7589684,9.17953597 C94.2815027,10.7851615 94.2815027,13.3014335 92.7589684,14.9070591 C92.0204707,15.6816261 90.9901475,16.1097181 89.9201961,16.086549 L89.9251939,16.086549 L89.9251939,16.086549 Z M87.9785358,9.98168907 C86.9193832,11.1520421 86.9193832,12.9345529 87.9785358,14.104906 C89.055542,15.1777887 90.7973447,15.1777887 91.874351,14.104906 C92.9323135,12.9331719 92.9296121,11.1502118 91.8681037,9.98168907 C90.7965232,8.90895059 89.055114,8.90895059 87.9785358,9.98168907 L87.9785358,9.98168907 Z M91.1696557,32.0433985 C91.1777483,34.202578 89.8833182,36.1536544 87.8908144,36.98555 C85.8983107,37.8174457 83.6008153,37.3660439 82.0711703,35.8421319 C80.5415253,34.31822 80.0814996,32.0224358 80.9059077,30.0268224 C81.7303157,28.0312089 83.6765178,26.7294617 85.8357124,26.7294465 C87.2530734,26.7073934 88.6190698,27.2596784 89.6228278,28.2606122 C90.6265858,29.261546 91.1827174,30.6259809 91.1646579,32.0433985 L91.1696557,32.0433985 L91.1696557,32.0433985 Z M85.8357124,28.8222914 C84.5123283,28.7278169 83.2658674,29.4525619 82.6933151,30.6494139 C82.1207628,31.846266 82.33869,33.2715477 83.2427217,34.2426284 C84.1467534,35.2137091 85.5528299,35.5328781 86.7875189,35.0472712 C88.0222079,34.5616643 88.8341284,33.3701505 88.8344156,32.0433985 C88.8775728,31.2141675 88.5821524,30.4029572 88.0158558,29.7956724 C87.4495592,29.1883876 86.660936,28.8370919 85.8307146,28.8222914 L85.8357124,28.8222914 L85.8357124,28.8222914 Z M80.9478262,15.7941755 L80.9478262,15.9191214 L79.7346008,15.9191214 L79.7346008,9.34321518 L77.6405064,9.34321518 L77.6405064,8.17247147 L83.0369228,8.17247147 L83.0369228,9.34321518 L80.9428284,9.34321518 L80.9428284,15.7941755 L80.9478262,15.7941755 L80.9478262,15.7941755 Z M75.5876442,15.9191214 L75.4626982,15.9191214 L75.4626982,8.17247147 L76.6746741,8.17247147 L76.6746741,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 L75.5876442,15.9191214 Z M79.52844,32.0371512 C79.5370392,34.1970611 78.2424922,36.1490549 76.2494097,36.9814653 C74.2563272,37.8138756 71.9579919,37.362448 70.4278281,35.8380191 C68.8976642,34.3135903 68.4376096,32.0169663 69.2625301,30.0207722 C70.0874506,28.0245781 72.0345697,26.7227105 74.1944967,26.7231992 C75.6138156,26.6997406 76.9820945,27.252272 77.9871357,28.2547235 C78.9921768,29.2571749 79.5482366,30.6240237 79.52844,32.0433985 L79.52844,32.0371512 L79.52844,32.0371512 Z M74.1944967,28.8160441 C72.8711125,28.7215696 71.6246517,29.4463146 71.0520994,30.6431666 C70.4795471,31.8400187 70.6974742,33.2653004 71.6015059,34.2363811 C72.5055376,35.2074618 73.9116141,35.5266308 75.1463032,35.0410239 C76.3809922,34.555417 77.1929127,33.3639032 77.1931999,32.0371512 C77.2345906,31.2098229 76.9390613,30.401016 76.3740386,29.7952647 C75.809016,29.1895133 75.0227012,28.8384931 74.1944967,28.8222914 L74.1944967,28.8160441 L74.1944967,28.8160441 Z M69.8913576,15.7941755 L69.8913576,15.9191214 L68.6768828,15.9191214 L68.6768828,9.34321518 L66.5827884,9.34321518 L66.5827884,8.17247147 L71.985452,8.17247147 L71.985452,9.34321518 L69.8913576,9.34321518 L69.8913576,15.7941755 L69.8913576,15.7941755 L69.8913576,15.7941755 Z M60.351733,23.1060134 C57.0394569,23.1060134 54.3543266,25.7911438 54.3543266,29.1034199 C54.3543266,32.415696 57.0394569,35.1008263 60.351733,35.1008263 C61.911274,35.1640272 63.4248461,34.5649522 64.518681,33.4515395 C65.2354102,32.670278 65.6676971,31.6701194 65.7456504,30.6127671 L60.351733,30.6127671 L60.351733,28.3637397 L67.9397016,28.3637397 C68.0269136,28.8279579 68.0687593,29.2995593 68.0646475,29.7718808 C68.1283197,31.7175966 67.4272681,33.6108394 66.1117421,35.0458501 C64.6136241,36.6071212 62.5174198,37.4502439 60.3554814,37.3610988 C57.38266,37.3976772 54.6198728,35.8326549 53.1228026,33.2640394 C51.6257325,30.6954239 51.6257325,27.5201621 53.1228026,24.9515465 C54.6198728,22.382931 57.38266,20.8179088 60.3554814,20.8544871 C62.4700022,20.8223125 64.5085325,21.6423177 66.0117853,23.1297532 L64.4199737,24.7153175 C63.3290662,23.6635564 61.866938,23.0851715 60.351733,23.1060134 L60.351733,23.1060134 L60.351733,23.1060134 Z M61.202615,8.17247147 L65.8256158,8.17247147 L65.8256158,9.34321518 L62.4208382,9.34321518 L62.4208382,11.4672966 L65.4995068,11.4672966 L65.4995068,12.6167995 L62.4208382,12.6167995 L62.4208382,14.740881 L65.8356115,14.740881 L65.8356115,15.9191214 L61.2126107,15.9191214 L61.2126107,8.17247147 L61.202615,8.17247147 L61.202615,8.17247147 Z M59.1047722,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 C58.362729,15.7061967 57.3293999,16.1227078 56.2622515,16.0840501 C54.0306055,16.0840501 52.2214989,14.2749436 52.2214989,12.0432975 C52.2214989,9.81165151 54.0306055,8.00254496 56.2622515,8.00254496 C56.8081716,8.00062413 57.3486772,8.11076649 57.8503147,8.32615501 C58.319951,8.51770113 58.7377625,8.81736104 59.0697874,9.20077678 L59.1435055,9.28948842 L59.0622906,9.36945384 L58.3063675,10.1253769 L58.2189053,10.0216718 C57.7341734,9.45029146 57.0105226,9.13791092 56.2622515,9.17703705 C54.9671893,9.17687941 53.8320222,10.0429715 53.4901083,11.2920836 C53.1481944,12.5411957 53.6840183,13.8646806 54.798621,14.5241099 C55.9132237,15.1835391 57.3311944,15.0159721 58.261387,14.1149017 C58.5935495,13.7564153 58.7951569,13.296416 58.8336395,12.8092163 L56.1310582,12.8092163 L56.1310582,11.658464 L59.9931381,11.658464 L60.01188,11.7621691 C60.0471602,11.9559383 60.0651399,12.1524609 60.0656067,12.3494151 C60.111529,13.307955 59.7629594,14.2434942 59.1010239,14.9382956 L59.1010239,14.9382956 L59.1047722,14.9382956 L59.1047722,14.9382956 Z"
                          id="Text"
                          fill="#FFFFFF"
                          mask="url(#mask-2)"
                        />
                        <path
                          d="M15.9938175,41 C16.1330287,40.9450684 16.2683395,40.879997 16.3987426,40.8052703 L34.9704194,30.2486714 L30.5673087,25.6830961 L15.9938175,41 L15.9938175,41 L15.9938175,41 Z M35.7923475,29.7821131 L31.1991699,25.0191646 L35.7876859,20.1967398 L42.1067218,23.7761104 C43.6634906,24.6572404 42.8672013,25.7604152 42.1067218,26.1928298 L35.7923475,29.7821131 L35.7923475,29.7821131 L35.7923475,29.7821131 Z M34.9659696,19.7317234 L16.3987426,9.21169149 C16.1969509,9.10052181 15.9772574,9.02877096 15.7505657,9 L30.5630708,24.3594185 L34.9666053,19.7315032 L34.9666053,19.7315032 L34.9659696,19.7317234 Z M14.661654,9.18966324 L29.9312096,25.0231297 L14.7349685,40.9936118 C14.2914791,40.7015172 14.0537365,40.0684253 14.0497106,39.248754 C14.0420825,37.6796817 13.9213042,12.770796 13.9121928,10.9014786 C13.9077431,10.0064708 14.2217667,9.4597296 14.661654,9.18966324 L14.661654,9.18966324 Z"
                          id="Logo"
                          fill="#FFFFFF"
                          mask="url(#mask-2)"
                        />
                      </g>
                    </g>
                  </svg>
                </a>
              </div>
            </div>
          </div>
          <div className="row2">
            <div className="footer-item">
              <p>
                2021-ci ildən xidmətinizdə olan “Reminant” onlayn mağazasının
                <br /> əsas fəaliyyət sahəsi müxtəlif dizaynlı geyimlərin topdan
                və pərakəndə satışıdır. <br />
                Müştərilər həm saytımızda yerləşdirilmiş hazır dizaynlı
                məhsulları sifariş edə,
                <br /> həm də öz istəklərinə uyğun istənilən dizaynla geyimlər
                hazırlatdıra bilərlər.
              </p>
              <div className="btn-flex">
                <a
                  target="_blank"
                  href="https://www.instagram.com/reminantbaku/"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={38}
                    height={38}
                    viewBox="0 0 24 24"
                    style={{ fill: 'rgba(26, 21, 21, 0.878)' }}
                  >
                    <path d="M11.999 7.377a4.623 4.623 0 1 0 0 9.248 4.623 4.623 0 0 0 0-9.248zm0 7.627a3.004 3.004 0 1 1 0-6.008 3.004 3.004 0 0 1 0 6.008z"></path>
                    <circle cx="16.806" cy="7.207" r="1.078" />
                    <path d="M20.533 6.111A4.605 4.605 0 0 0 17.9 3.479a6.606 6.606 0 0 0-2.186-.42c-.963-.042-1.268-.054-3.71-.054s-2.755 0-3.71.054a6.554 6.554 0 0 0-2.184.42 4.6 4.6 0 0 0-2.633 2.632 6.585 6.585 0 0 0-.419 2.186c-.043.962-.056 1.267-.056 3.71 0 2.442 0 2.753.056 3.71.015.748.156 1.486.419 2.187a4.61 4.61 0 0 0 2.634 2.632 6.584 6.584 0 0 0 2.185.45c.963.042 1.268.055 3.71.055s2.755 0 3.71-.055a6.615 6.615 0 0 0 2.186-.419 4.613 4.613 0 0 0 2.633-2.633c.263-.7.404-1.438.419-2.186.043-.962.056-1.267.056-3.71s0-2.753-.056-3.71a6.581 6.581 0 0 0-.421-2.217zm-1.218 9.532a5.043 5.043 0 0 1-.311 1.688 2.987 2.987 0 0 1-1.712 1.711 4.985 4.985 0 0 1-1.67.311c-.95.044-1.218.055-3.654.055-2.438 0-2.687 0-3.655-.055a4.96 4.96 0 0 1-1.669-.311 2.985 2.985 0 0 1-1.719-1.711 5.08 5.08 0 0 1-.311-1.669c-.043-.95-.053-1.218-.053-3.654 0-2.437 0-2.686.053-3.655a5.038 5.038 0 0 1 .311-1.687c.305-.789.93-1.41 1.719-1.712a5.01 5.01 0 0 1 1.669-.311c.951-.043 1.218-.055 3.655-.055s2.687 0 3.654.055a4.96 4.96 0 0 1 1.67.311 2.991 2.991 0 0 1 1.712 1.712 5.08 5.08 0 0 1 .311 1.669c.043.951.054 1.218.054 3.655 0 2.436 0 2.698-.043 3.654h-.011z"></path>
                  </svg>
                </a>
                <a
                  target="_blank"
                  href="https://www.facebook.com/profile.php?id=100076146334062"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={38}
                    height={38}
                    viewBox="0 0 24 24"
                    style={{ fill: 'rgba(26, 21, 21, 0.878)' }}
                  >
                    <path d="M13.397 20.997v-8.196h2.765l.411-3.209h-3.176V7.548c0-.926.258-1.56 1.587-1.56h1.684V3.127A22.336 22.336 0 0 0 14.201 3c-2.444 0-4.122 1.492-4.122 4.231v2.355H7.332v3.209h2.753v8.202h3.312z"></path>
                  </svg>
                </a>
                <a target="_blank" href="https://wa.me/994508986473">
                  {' '}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width={38}
                    height={38}
                    viewBox="0 0 24 24"
                    style={{ fill: 'rgba(26, 21, 21, 0.878)' }}
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M18.403 5.633A8.919 8.919 0 0 0 12.053 3c-4.948 0-8.976 4.027-8.978 8.977 0 1.582.413 3.126 1.198 4.488L3 21.116l4.759-1.249a8.981 8.981 0 0 0 4.29 1.093h.004c4.947 0 8.975-4.027 8.977-8.977a8.926 8.926 0 0 0-2.627-6.35m-6.35 13.812h-.003a7.446 7.446 0 0 1-3.798-1.041l-.272-.162-2.824.741.753-2.753-.177-.282a7.448 7.448 0 0 1-1.141-3.971c.002-4.114 3.349-7.461 7.465-7.461a7.413 7.413 0 0 1 5.275 2.188 7.42 7.42 0 0 1 2.183 5.279c-.002 4.114-3.349 7.462-7.461 7.462m4.093-5.589c-.225-.113-1.327-.655-1.533-.73-.205-.075-.354-.112-.504.112s-.58.729-.711.879-.262.168-.486.056-.947-.349-1.804-1.113c-.667-.595-1.117-1.329-1.248-1.554s-.014-.346.099-.458c.101-.1.224-.262.336-.393.112-.131.149-.224.224-.374s.038-.281-.019-.393c-.056-.113-.505-1.217-.692-1.666-.181-.435-.366-.377-.504-.383a9.65 9.65 0 0 0-.429-.008.826.826 0 0 0-.599.28c-.206.225-.785.767-.785 1.871s.804 2.171.916 2.321c.112.15 1.582 2.415 3.832 3.387.536.231.954.369 1.279.473.537.171 1.026.146 1.413.089.431-.064 1.327-.542 1.514-1.066.187-.524.187-.973.131-1.067-.056-.094-.207-.151-.43-.263"
                    ></path>
                  </svg>{' '}
                </a>
              </div>
            </div>
            <div className="footer-list">
              <h5 className="footer-title">Menyu</h5>
              <ul>
                <li>
                  <a href="./home.html"> Ana Səhifə </a>
                </li>
                <li>
                  <a href="./qanda.html"> Sual&amp;Cavab </a>
                </li>
                <li>
                  <a href="./about.html"> Haqqımızda </a>
                </li>
                <li>
                  <a href="./contact.html">Əlaqə </a>
                </li>
              </ul>
            </div>
            <div className="footer-list">
              <h5 className="footer-title">Əlavə</h5>
              <ul>
                <li>
                  <a href="./install.html"> Yüklə </a>
                </li>
              </ul>
            </div>
            {/* <div class="footer-list">
          <h5 class="footer-title">Product</h5>
          <ul>
            <li><a href="#"> ArriveSafe App </a></li>
            <li><a href="#"> How the app works </a></li>
            <li><a href="#"> features </a></li>
            <li><a href="#"> FAQ & Support </a></li>
          </ul>
        </div>
        <div class="footer-list">
          <h5 class="footer-title">Product</h5>
          <ul>
            <li><a href="#"> ArriveSafe App </a></li>
            <li><a href="#"> How the app works </a></li>
            <li><a href="#"> features </a></li>
            <li><a href="#"> FAQ & Support </a></li>
          </ul>
        </div> */}
          </div>
        </div>
        <div className="footer-bottom">
          <span> © 2021 Reminant VÖEN: 2007087522 </span>
        </div>
      </footer>
    </>
  );
}
